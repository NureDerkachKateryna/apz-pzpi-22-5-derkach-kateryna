    ...
    public class SkincareRoutineService : ISkincareRoutineService
    {
        ...
        public async Task<List<SkincareRoutineDto>> GenerateUserRoutinesAsync(string userId)
        {
            try
            {
                ...
                List<SkincareRoutineDto> skincareRoutines = await GetSkincareRoutinesByUserAsync(userId);

                if (skincareRoutines.Count > 0)
                {
                    foreach (var skincareRoutine in skincareRoutines)
                    {
                        await DeleteSkincareRoutineAsync(skincareRoutine.SkincareRoutineId);
                    }
                }                
